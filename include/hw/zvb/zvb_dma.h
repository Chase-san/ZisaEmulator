...
/**
 * @brief DMA descriptors array pointed by the `desc_addr` field in the
 * controller
 */
typedef struct {
    uint8_t rd_addr[3];    // Source address (24-bit LE)
    uint8_t wr_addr[3];    // Destination address (24-bit LE)
...
// packed structures aren't really portable, so making do.
