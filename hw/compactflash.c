...
static uint8_t io_read(device_t *dev, uint32_t addr) {
    ...
    switch (addr) {
        ...
        case IDE_REG_ERROR:
            return cf->error;
        case IDE_REG_SEC_CNT:
            return cf->sec_cnt & 0xff;
        case IDE_REG_LBA_0:
            return cf->lba_0;
        case IDE_REG_LBA_8:
            return cf->lba_8;
        ...
        case IDE_REG_LBA_24:
            return cf->lba_24;
        case IDE_REG_DATA:
            return compactflash_read_data(cf);
        default:
            return 0;
    }
}
...
static void io_write(device_t *dev, uint32_t addr, uint8_t value) {
    ...
    switch (addr) {
        case IDE_REG_DATA:
            compactflash_write_data(cf, value);
            break;
        case IDE_REG_COMMAND:
            compactflash_process_command(cf, value);
            break;
        case IDE_REG_FEATURE:
            cf->feature = value;
            break;
        case IDE_REG_SEC_CNT:
            cf->sec_cnt = value == 0 ? 256 : value;
            break;
        case IDE_REG_LBA_0:
            cf->lba_0 = value;
            break;
...
static uint32_t compactflash_data_ofs(compactflash_t *cf) {
    ...
    uint32_t sector = ((cf->lba_24 & 0xF) << 24) | (cf->lba_16 << 16) | (cf->lba_8 << 8) | (cf->lba_0 << 0);
...
static void compactflash_read_sector_buffer(compactflash_t *cf) {
    if (fseek(cf->fd, cf->data_ofs, SEEK_SET) == -1) {
        perror_exit(cf, "seek()");
    }
    int bytes_read = fread(cf->sector_buffer, 1, 512, cf->fd);
    if (bytes_read == -1) {
        perror_exit(cf, "read()");
    }
    if (bytes_read < 512) {
...
static void compactflash_write_sector_buffer(compactflash_t *cf) {
    ...
    if (fwrite(cf->sector_buffer, 1, 512, cf->fd) != 512) {
...
int compactflash_init(compactflash_t *cf, const char *file_name) {
    ...
    FILE *fd = fopen(file_name, "r+b");
    if (fd == NULL) {
        log_err_printf("[COMPACTFLASH] Could not open file");
    ...
    *cf = (compactflash_t){// Initialize all fields
                           .fd = fd,
                           .size = 8,
                           .total_sectors = total_sectors,
                           .file_name = zstrdup(file_name),
